...
"""AUC score""" 
def getAUC(testImage, resultImage):    
    """Compute AUROC."""
    image = sitk.ReadImage(testImage)
    testArray = sitk.GetArrayFromImage(image).flatten()
    image = sitk.ReadImage(resultImage)
    resultArray = sitk.GetArrayFromImage(image).flatten()
    
    fpr, tpr, thresholds = M.roc_curve(y_true = testArray, y_score = resultArray, pos_label = 1)
    return M.auc(fpr, tpr)

def getImagesABVIB(testFilename, resultFilename):
    """Return the test and result images, thresholded and non-WMH masked."""
    testImage   = sitk.ReadImage(testFilename)
    resultImage = sitk.ReadImage(resultFilename)
    assert testImage.GetSize() == resultImage.GetSize()
    ...
    # Remove non-WMH from the test and result images, since we don't evaluate on that
    maskedTestImage = sitk.BinaryThreshold(testImage, 0.5,  300, 1, 0) # WMH == 1    
    nonWMHImage     = sitk.BinaryThreshold(testImage, 1.5,  2.5, 0, 1) # non-WMH == 2
    maskedResultImage = sitk.Mask(resultImage, nonWMHImage)
